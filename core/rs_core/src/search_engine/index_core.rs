...
impl Trie {
    ...
    pub fn search(&self, query: &str, n: usize, doc_type: Option<i32>) -> Vec<i32> {
        self.search_node(query)
            .map(|node| {
                self.filter_and_sort_results(node, n, |dtype| doc_type.map_or(true, |t| t == dtype))
            })
            .unwrap_or_default()
    }

    pub fn search_by_type_list(
        &self,
        query: &str,
        n: usize,
...
mod tests {
    ...
    fn test_search() {
        ...
        // 测试带有 doc_type 的搜索
        let results = trie.search("应用", 5, Some(3));
    ...
    fn test_case_insensitive_search() {
        ...
        let results = trie.search("APPly", 5, Some(2));
    ...
    fn test_search_by_type_list() {
        ...
        // 测试单一类型
        let results = trie.search_by_type_list("app", 5, Some(vec![1]));
        ...
        // 测试所有类型（不提供类型列表）
        let results = trie.search_by_type_list("app", 5, None);
        assert_eq!(results, vec![5, 4, 2, 3, 1]);

        // 测试空类型列表
        let results: Vec<i32> = trie.search_by_type_list("app", 5, Some(vec![]));
        assert_eq!(results, Vec::<i32>::new());

        // 测试不存在的类型
        let results: Vec<i32> = trie.search_by_type_list("app", 5, Some(vec![4]));
        assert_eq!(results, Vec::<i32>::new());

        // 测试限制结果数量
        let results = trie.search_by_type_list("app", 2, Some(vec![1, 2]));
        assert_eq!(results, vec![4, 2]);

        // 测试中文搜索
        let results = trie.search_by_type_list("应用", 5, Some(vec![1, 2, 3]));
